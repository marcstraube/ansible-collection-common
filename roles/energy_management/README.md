...
power manager (PPD or TLP), battery charge thresholds, and backlight access.
...
| EL 9 (Rocky, Alma, RHEL)  | PPD via power-profiles-daemon; no HibernateDelaySec (systemd < 256) |
| EL 10 (Rocky, Alma, RHEL) | PPD via tuned + tuned-ppd; tp_smapi not available                   |
...
|-----------------------------|---------|-----------------------------------|
| `energy_management_enabled` | `true`  | Enable the energy_management role |

### System Type

| Variable                        | Default     | Description                                      |
|---------------------------------|-------------|--------------------------------------------------|
| `energy_management_system_type` | `'desktop'` | System type: `desktop` or `laptop` (auto-detect) |
...
| Variable                                             | Default     | Description                     |
|------------------------------------------------------|-------------|---------------------------------|
| `energy_management_logind_enabled`                   | `true`      | Enable logind drop-in           |
...
| Variable                                         | Default              | Description                  |
|--------------------------------------------------|----------------------|------------------------------|
| `energy_management_sleep_enabled`                | `true`               | Enable sleep drop-in         |
...
| `energy_management_suspend_mode`                 | `''`                 | Suspend mode                 |
...
| `energy_management_hibernate_delay_sec`          | `3600`               | Delay before hibernate       |
...
| `energy_management_tlp_platform_profile_on_bat`       | `low-power`           | Platform profile on bat   |
| `energy_management_tlp_wifi_pwr_on_ac`                | `off`                 | WiFi power saving on AC   |
| `energy_management_tlp_wifi_pwr_on_bat`               | `on`                  | WiFi power saving on bat  |
...
| `energy_management_tlp_nmi_watchdog`                  | `0`                   | NMI watchdog (0 = off)    |
| `energy_management_tlp_extra_settings`                | `{}`                  | Additional raw key=values |
...
| `energy_management_battery_tp_smapi_enabled` | `false` | Enable tp_smapi for older ThinkPads |
...
PPD and TLP are mutually exclusive power managers. Selecting one automatically
removes the other. This role does NOT use the standard ternary service manage
