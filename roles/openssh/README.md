...
Host key types are configurable; algorithm lists (HostKeyAlgorithms, CASignatureAlgorithms,
PubkeyAcceptedAlgorithms) are auto-derived from the selected key types via an algorithm map,
or can be overridden explicitly.
...
| Platform       | OpenSSH Version | Notes                                 |
| -------------- | --------------- | ------------------------------------- |
| Arch Linux     | 10.3p1          | Rolling, latest upstream              |
| Debian Trixie  | 10.0p1          | DSA removed, post-quantum KEM default |
| Rocky Linux 9  | 8.7p1           | No RequiredRSASize, no penalty system |
| Rocky Linux 10 | 9.9p1           | Full feature set                      |
...
| ------------------------ | --------- | ------------------------- |
| `openssh_server_enabled` | `true`    | Enable/disable SSH server |
| `openssh_server_state`   | `started` | Service state             |
...
| `openssh_apparmor_enabled`  | `false`  | Enforce AppArmor profile |
| `openssh_install_ssh_audit` | `true`   | Install ssh-audit tool   |
